package lm.group.testing_f_tmp;
...
public class MyReducer implements Reducer {
    ...
    // 1212 对 购物车 ，购买数的影响  特征 
//    i_day_dtx_avg_tpy_days_count; //平均每天被tpy次数
    // d170~d172
    private int[] tpx_1212 ;
    private double[] tpx_1212_to_avg;
...
//            int dt_d = Integer.parseInt(parts[1]); 
            ...
            // 1212 to avg features
            if(dt.equals("2014-12-12")) 
            	{tpx_1212[tp-1] +=1 ;}
        ...
        get1212ToAvgFeatures();
        
        result.set(0,key.get("iid"));
        
        int counter =1;
        ArrayList fList = new ArrayList(172);
...
//        
//        for (int i =1 ; i<4;i++){
//      		 result.set(counter, tpx_1212[i] );counter++;	 
//      	 }
//        
//        for (int i =0 ; i<3;i++){
//   		 result.set(counter, tpx_1212_to_avg[i] );counter++;	 
//   	 	} 	
    ...
    public void get1212ToAvgFeatures(){
    	 for (int i =0 ; i<3;i++){
    		 tpx_1212_to_avg[i] = tpx_1212[1+i]*1.0 / (i_day_dtx_avg_tpy_days_count[1][i+1] + tpx_1212[1+i] +MC0 );
    	 } 	
    }
...
//	    	i_day_dtx_cart2view_ratio[i] = i_day_dtx_tpy_amount[i][2]*1.0 / ( i_day_dtx_tpy_amount[i][0] + MC0 ) ;
	    	i_day_dtx_cart2view_ratio[i] = i_day_dtx_tpy_amount[i][2]*1.0 / (i_day_dtx_tpy_amount[i][2]*1.0 + i_day_dtx_tpy_amount[i][0] + MC0 ) ;
//	    	i_day_dtx_cart2view_ratio[i] = sigmoid(  i_day_dtx_tpy_amount[i][2]*1.0 / ( i_day_dtx_tpy_amount[i][0] + MC0 ) /10 );
	    	//cart数/view数 * log(cart)
	    	i_day_dtx_cart2view_ratio_log_cart[i] = i_day_dtx_cart2view_ratio[i] * Math.log(i_day_dtx_tpy_amount[i][2]*1.0 + MC0 + 1 );
	    	//cart数/buy数
//	 	    i_day_dtx_cart2buy_ratio[i] = i_day_dtx_tpy_amount[i][2]*1.0 / (  i_day_dtx_tpy_amount[i][3] + MC0 ) ;
	 	    i_day_dtx_cart2buy_ratio[i] = i_day_dtx_tpy_amount[i][2]*1.0 / ( i_day_dtx_tpy_amount[i][2]*1.0 + i_day_dtx_tpy_amount[i][3] + MC0 ) ;
//	 	    i_day_dtx_cart2buy_ratio[i] = sigmoid(i_day_dtx_tpy_amount[i][2]*1.0 / (  i_day_dtx_tpy_amount[i][3] + MC0) /10) ;
	 	    //cart数/buy数 * log(cart)
	 	    i_day_dtx_cart2buy_ratio_log_cart[i] = i_day_dtx_cart2buy_ratio[i] * Math.log(i_day_dtx_tpy_amount[i][2]*1.0 + MC0 + 1 );
	 	    //collect数/view数
//	 	    i_day_dtx_collect2view_ratio[i] = i_day_dtx_tpy_amount[i][1]*1.0 / ( i_day_dtx_tpy_amount[i][0] + MC0 ) ;
	 	    i_day_dtx_collect2view_ratio[i] = i_day_dtx_tpy_amount[i][1]*1.0 / (i_day_dtx_tpy_amount[i][1]*1.0 + i_day_dtx_tpy_amount[i][0] + MC0 ) ;
//	 	    i_day_dtx_collect2view_ratio[i] = sigmoid( i_day_dtx_tpy_amount[i][1]*1.0 / ( i_day_dtx_tpy_amount[i][0] + MC0 ) /10) ;
	 	    //collect数/view数* log(collect)
	 	    i_day_dtx_collect2view_ratio_log_collect[i] = i_day_dtx_collect2view_ratio[i] * Math.log(  i_day_dtx_tpy_amount[i][1]*1.0  + MC0 + 1   );
	 	    //collect数/buy数
//	 	    i_day_dtx_collect2buy_ratio[i] = i_day_dtx_tpy_amount[i][1]*1.0 / ( i_day_dtx_tpy_amount[i][3] + MC0 ) ;
	 	    i_day_dtx_collect2buy_ratio[i] = i_day_dtx_tpy_amount[i][1]*1.0 / (i_day_dtx_tpy_amount[i][1]*1.0 + i_day_dtx_tpy_amount[i][3] + MC0 ) ;
//	 	    i_day_dtx_collect2buy_ratio[i] = sigmoid(i_day_dtx_tpy_amount[i][1]*1.0 / ( i_day_dtx_tpy_amount[i][3] + MC0 ) /10) ;
	 	    //collect数/buy数* log(collect)
	 	    i_day_dtx_collect2buy_ratio_log_collect[i] = i_day_dtx_collect2buy_ratio[i] * Math.log(  i_day_dtx_tpy_amount[i][1]*1.0  + MC0  + 1  );
	    }
